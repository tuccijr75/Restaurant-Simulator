...
- 120/120 self-test PASS across all 10 scenarios, seed 12345.
- DT lane queue is now bounded by window labor: multi_rush shows 214s DT SOS and 99 balked cars (lane saturation) — strain restored causally after RS-ST-001's smart deployment removed it.
- CarAgent window hold now models real service labor, not a synthetic pause.
- Honest finding: service capacity is the binding constraint at peak (as in real QSR); kitchen rarely saturates first. Matches Intouch decomposition where wait dominates total time.
