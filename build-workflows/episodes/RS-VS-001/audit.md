...
- All visual changes are presentation-layer: zero sim-state writes; engine hashes unchanged (verified via headless suite after world edits).
- Godot-only files cannot compile in the build container — APIs written conservatively against Godot 4.6 C# (CpuParticles3D, ProceduralSkyMaterial runtime color writes, Label3D). First in-editor run should smoke-test: day/night sweep at 10x, KDS boards updating, click-to-open panels.
