...
public class SimRunState{
 ...
 public string Scenario="normal_day",RecentEvents="",RecentJsonl="",AllJsonl="",WasteLedger="",StaffingLedger="",TraceLedger="",ValidationLedger="",OverloadLedger="",EquipmentLedger="",ItemLedger="",TaskLedger="",ValidationStatus="OK";
 ...
 void AddOrder(){
  ...
  var t=new Ticket{Id=Orders,OrderId=oid,Channel=ch,Created=Minute};
  activeTickets.Add(t);
  if(fried){MainsSold++;AddItem(t,items["fried_main"],new Dictionary<string,double>{{"main_protein",1},{"prep_pack",.5}});}else{MainsSold++;AddItem(t,items["grilled_main"],new Dictionary<string,double>{{"main_protein",1},{"prep_pack",.5}});}
  if(hasFries){FriesSold++;AddItem(t,items["side"],new Dictionary<string,double>{{"side_base",1}});}
  if(hasDrink){DrinksSold++;AddItem(t,items["beverage"],new Dictionary<string,double>{{"drink_mix",.25}});}
 ...
 void AddItem(Ticket t,ItemSpec spec,Dictionary<string,double> draw){
  var cook=CreateTask(t,spec.Id,"cook",spec.Station,spec.Family,spec.CookSeconds,0);
  var previous=cook.Id;
  if(spec.AssemblySeconds>0)previous=CreateTask(t,spec.Id,"assembly","assembly","assembly",spec.AssemblySeconds,previous).Id;
  if(spec.ExpoSeconds>0)CreateTask(t,spec.Id,"expo","expo","expo",spec.ExpoSeconds,previous);
  QueueReadyTasks(t);
  foreach(var kv in draw)inventory[kv.Key]=Math.Max(0,inventory.GetValueOrDefault(kv.Key)-kv.Value);
  ItemSeq++;
  var primary=cook.EquipmentId==""?"pending":cook.EquipmentId;
  ItemLedger+=$"{ItemSeq} {TimeText} order {t.OrderId} item {spec.Id} primary_equipment {primary} dependency cook->assembly->expo cook {Num(spec.CookSeconds)}s hold {Num(spec.HoldMinutes)}m assembly {Num(spec.AssemblySeconds)}s expo {Num(spec.ExpoSeconds)}s price ${spec.Price:0.00}\n";
  Emit("item.sold",$"{{\"order_id\":\"{t.OrderId}\",\"item_id\":\"{spec.Id}\",\"quantity\":1,\"station_ids\":[\"{primary}\"],\"inventory_draw\":{DrawJson(draw)}}}");
 }
 ...
 EquipTask CreateTask(Ticket t,string item,string phase,string station,string family,double work,int dependency){
  ...
  t.Tasks.Add(task);
  TaskLedger+=$"{TimeText} created task_{task.Id:000000} order {task.OrderId} item {item} phase {phase} station {station} family {family} dependency {(dependency==0?"none":$"task_{dependency:000000}")} work {Num(work)}s\n";
  return task;
 }
 ...
 void QueueReadyTasks(Ticket t){
  foreach(var task in t.Tasks){
   ...
   TaskLedger+=$"{TimeText} queued task_{task.Id:000000} order {task.OrderId} item {task.ItemId} phase {task.Phase} equipment {unit.Id} load {Num(EquipmentLoad(unit))} cap_min {Num(EquipmentCapacity(unit))}\n";
   EquipmentLedger+=$"{TimeText} assign order {task.OrderId} item {task.ItemId} phase {task.Phase} equipment {unit.Id} family {task.Family} work {Num(task.WorkSeconds)}s load {Num(EquipmentLoad(unit))} cap_min {Num(EquipmentCapacity(unit))}\n";
  }
 }
 ...
 void StartNext(EquipmentUnit e){
  ...
  TaskLedger+=$"{TimeText} started task_{e.Active.Id:000000} order {e.Active.OrderId} item {e.Active.ItemId} phase {e.Active.Phase} equipment {e.Id}\n";
 }
 void CompleteTask(EquipmentUnit e){
  if(e.Active==null)return;
  var task=e.Active;task.Done=true;task.Remaining=0;
  TaskLedger+=$"{TimeText} completed task_{task.Id:000000} order {task.OrderId} item {task.ItemId} phase {task.Phase} equipment {e.Id}\n";
  e.Active=null;
  var t=FindTicket(task.TicketId);
  if(t!=null)QueueReadyTasks(t);
 }
 ...
 bool TicketDone(Ticket t){if(t.Tasks.Count==0)return false;foreach(var task in t.Tasks)if(!task.Done)return false;return true;}
 ...
 void EnsureEquipment(){if(equipmentReady)return;equipmentReady=true;AddEquipment("fryer_fries_1","fryer","fries",70);AddEquipment("fryer_fries_2","fryer","fries",70);AddEquipment("fryer_main_1","fryer","fried_main",70);AddEquipment("fryer_main_2","fryer","fried_main",70);AddEquipment("burger_press_1","grill","grilled_main",95);AddEquipment("soda_1","beverage","beverage",45);AddEquipment("soda_2","beverage","beverage",45);AddEquipment("soda_3","beverage","beverage",45);AddEquipment("soda_4","beverage","beverage",45);AddEquipment("assembly_rail_1","assembly","assembly",60);AddEquipment("assembly_rail_2","assembly","assembly",60);AddEquipment("expo_lane_1","expo","expo",65);AddEquipment("expo_lane_2","expo","expo",65);items["fried_main"]=new ItemSpec{Id="fried_main",Family="fried_main",Station="fryer",CookSeconds=330,HoldMinutes=20,AssemblySeconds=60,ExpoSeconds=25,Price=6.25};items["grilled_main"]=new ItemSpec{Id="grilled_main",Family="grilled_main",Station="grill",CookSeconds=150,HoldMinutes=15,AssemblySeconds=55,ExpoSeconds=25,Price=6.35};items["side"]=new ItemSpec{Id="side",Family="fries",Station="fryer",CookSeconds=180,HoldMinutes=7,AssemblySeconds=15,ExpoSeconds=5,Price=2.75};items["beverage"]=new ItemSpec{Id="beverage",Family="beverage",Station="beverage",CookSeconds=22,HoldMinutes=0,AssemblySeconds=0,ExpoSeconds=5,Price=2.35};}
 ...
 public string ItemCatalogJson=>"[{\"item_id\":\"fried_main\",\"equipment\":\"fryer_main_1|fryer_main_2\",\"cook_seconds\":330,\"hold_minutes\":20,\"dependency\":\"cook->assembly->expo\"},{\"item_id\":\"grilled_main\",\"equipment\":\"burger_press_1\",\"cook_seconds\":150,\"hold_minutes\":15,\"dependency\":\"cook->assembly->expo\"},{\"item_id\":\"side\",\"equipment\":\"fryer_fries_1|fryer_fries_2\",\"cook_seconds\":180,\"hold_minutes\":7,\"dependency\":\"cook->assembly->expo\"},{\"item_id\":\"beverage\",\"equipment\":\"soda_1|soda_2|soda_3|soda_4\",\"cook_seconds\":22,\"hold_minutes\":0,\"dependency\":\"pour->expo\"}]";
