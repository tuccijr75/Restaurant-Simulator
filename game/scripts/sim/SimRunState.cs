...
public class SimRunState{
 ...
 void AddItem(Ticket t,ItemSpec spec,Dictionary<string,double> draw){
  ...
  ItemLedger+=$"{ItemSeq} {TimeText} taken order {t.OrderId} item {spec.Id} primary_equipment {primary} dependency cook->assembly->expo cook {Num(spec.CookSeconds)}s hold {Num(spec.HoldMinutes)}m assembly {Num(spec.AssemblySeconds)}s expo {Num(spec.ExpoSeconds)}s price ${spec.Price:0.00}\n";
  Emit("item.taken",$"{{\"order_id\":\"{t.OrderId}\",\"item_id\":\"{spec.Id}\",\"quantity\":1,\"station_ids\":[\"{primary}\"],\"inventory_draw\":{DrawJson(draw)},\"status\":\"taken\"}}");
 }
 ...
 void CompleteTask(EquipmentUnit e){
  ...
  if(t!=null){QueueReadyTasks(t);if(task.Phase=="expo")EmitItemCompleted(t,task);}
 }
 void EmitItemCompleted(Ticket t,EquipTask task){
  var sec=(int)((Minute-t.Created+1440)%1440*60);
  ItemLedger+=$"{ItemSeq} {TimeText} completed order {t.OrderId} item {task.ItemId} final_equipment {task.EquipmentId} elapsed {sec}s\n";
